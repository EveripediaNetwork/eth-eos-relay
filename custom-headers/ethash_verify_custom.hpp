void verifyCustom(Strictness _s, BlockHeader const& _bi, BlockHeader const& _parent, bytesConstRef _block)
{
	...
	if (_parent)
	{
		...
		auto expected = calculateDifficultyCustom(_bi, _parent);
